...
from music_dataset import train_data_loader, test_data_loader, music_collate_fn, LABEL_NAMES_TASK_2
from models.HEAR import HEAR
from models.listMLE import listMLE
from torch.optim.lr_scheduler import ReduceLROnPlateau
import yaml
import numpy as np
from utils import calculate_results_task_2, save_networks, load_networks
...
parser.add_argument('--experiment_name', type=str, default='task_2')
...
def main(options):
    ...
    train_data = train_data_loader(pkl_path=options.get('train_data'), label_names=LABEL_NAMES_TASK_2)
    test_data = test_data_loader(pkl_path=options.get('test_data'), label_names=LABEL_NAMES_TASK_2)
    ...
    with open("config_task_2.yaml", 'r') as f:
    ...
    if options['load_checkpoint'] == True:
        ...
        model = load_networks(model, "log/models/test_task_2")
...
def train(epoch, model, train_loader, loss_smooth, optimizer, scheduler,
    ...
    for batch_idx, (audios, labels, audio_masks, partner_audios, partner_labels, partner_masks) in enumerate(train_loader):
        ...
        if (batch_idx + 1) % eval_interval == 0 or (batch_idx + 1) == total_steps:
            ...
            total_score = sum(results['mean'].values())
            best_score = sum(best_results['mean'].values())
...
def test(model, test_loader, epoch=0, step=0, **options):
    ...
    results = calculate_results_task_2(y_true, y_pred, **options)
    return results
